...
document.addEventListener('DOMContentLoaded', function () {
  ...
  const openAddAttendanceModalButton = document.getElementById(
  ...
  function setupEventListeners() {
    openAddAttendanceModalButton.addEventListener(
      'click',
      openAddAttendanceModal
    );
...
async function deleteActivity() {
  ...
  if (!currentActivityId) {
    showAlert(
      'No se ha seleccionado una actividad válida para eliminar.',
      'danger'
    );
    return;
  }
  ...
  try {
    const response = await fetch(
      `/api/assistance/delete/${currentActivityId}`,
      {
        method: 'DELETE',
      }
    );
